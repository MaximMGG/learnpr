...
engine_get_insert_quary :: #force_inline proc(
	db: ^Database,
	$T: typeid,
	mapping_name: string = "",
) -> string {
...
engine_select_struct :: proc(db: ^Database, $T: typeid, mapping_name: string = "") -> []T {
	...
	ti := tin.base.variant.(runtime.Type_Info_Struct)
	for j in 0 ..< rows {
		base := uintptr(&res_struct[j])
		for i in 0 ..< ti.field_count {
			switch ti.types[i].id {
			case i8:
				res := PQgetvalue(db.res, j, i)
				res_int, _ := strconv.parse_int(string(res), 10)
				res_i8 := i8(res_int)
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_i8, size_of(i8))
			case u8:
				res := PQgetvalue(db.res, j, i)
				res_int, _ := strconv.parse_int(string(res), 10)
				res_u8 := u8(res_int)
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_u8, size_of(u8))
			case i16:
				res := PQgetvalue(db.res, j, i)
				res_int, _ := strconv.parse_int(string(res), 10)
				res_i16 := i16(res_int)
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_i16, size_of(i16))
			case u16:
				res := PQgetvalue(db.res, j, i)
				res_int, _ := strconv.parse_int(string(res), 10)
				res_u16 := u16(res_int)
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_u16, size_of(u16))
			case i32:
				res := PQgetvalue(db.res, j, i)
				res_int, _ := strconv.parse_int(string(res), 10)
				res_i32 := i32(res_int)
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_i32, size_of(i32))
			case u32:
				res := PQgetvalue(db.res, j, i)
				res_int, _ := strconv.parse_int(string(res), 10)
				res_u32 := u32(res_int)
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_u32, size_of(u32))
			case i64:
				res := PQgetvalue(db.res, j, i)
				res_int, _ := strconv.parse_int(string(res), 10)
				res_i64 := i64(res_int)
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_i64, size_of(i64))
			case u64:
				res := PQgetvalue(db.res, j, i)
				res_int, _ := strconv.parse_int(string(res), 10)
				res_u64 := u64(res_int)
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_u64, size_of(u64))
			case int:
				res := PQgetvalue(db.res, j, i)
				res_int, _ := strconv.parse_int(string(res), 10)
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_int, size_of(int))
			case f32:
				res := PQgetvalue(db.res, j, i)
				res_float, _ := strconv.parse_f32(string(res))
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_float, size_of(f32))
			case f64:
				res := PQgetvalue(db.res, j, i)
				res_double, _ := strconv.parse_f64(string(res))
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_double, size_of(f64))
			case cstring:
				res := PQgetvalue(db.res, j, i)
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res, size_of(cstring))
			case string:
				res := PQgetvalue(db.res, j, i)
				res_string := string(res)
				mem.copy(rawptr(base + uintptr(ti.offsets[i])), &res_string, size_of(string))
			}
		}
	}
...
engine_select_struct_quary :: proc(
	db: ^Database,
	$T: typeid,
	mapping_name: string,
	quary: string,
...
engine_destroy :: proc(db: ^Database) {
	...
	for k, v in db.prepared_selects {
		delete(v)
	}
	free(db)
}
