...
pub const Texture = struct {
    ...
    pub fn bind(self: *Texture, slot: u32) void {
        //gl.glActiveTexture().?(@as(c_uint, @intCast(gl.GL_TEXTURE0)) + @as(c_uint, @intCast(slot)));
        gl.glActiveTexture().?( @intCast(gl.GL_TEXTURE0));
...
pub fn texture(file_path: []const u8) Texture {
    ...
    gl.glTexImage2D(gl.GL_TEXTURE_2D, 0, gl.GL_RGBA8, @intCast(t.width), @intCast(t.height), 0, gl.GL_RGBA, gl.GL_UNSIGNED_BYTE, t.local_buffer.ptr);
    gl.glBindTexture(gl.GL_TEXTURE_2D, 0);
